...
class TabF1(Metric):
    ...
    def __call__(self, y_true, y_score):
        return f1_score(y_true, (y_score[:, 1] > 0.5).astype(int))
...
class myInfer(inferServer):
    ...
    def predict(self, data):
        global fail
        if data is not None:
            dv = ss.transform(data.iloc[:, 1:].values)
            ret = np.zeros((data.shape[0], 5))
            for i in range(5):
                ret[:, i] = self.model[i].predict_proba(dv)[:, 1]
