...
class TPS(object):
    ...
    def evaluate(self, approxes, target, weight):
        ...
        for i in range(size):
            ...
            tt[i] = target[i]
        mask = ax <= tt
        e = np.exp(np.expm1(ax[mask]) / np.expm1(tt[mask]))
        error_sum = np.sum(e / (1 + e))
...
class myInfer(inferServer):
    ...
    def post_process(self, data):
        ...
        totl.update(set(data["serial_number"]))
        print(f"Total bad servers: {len(totl)}", file=sys.stderr)
        return ret
    ...
    def predict(self, data):
        ...
        if data is not None:
            ...
            for i in range(10):
                ret[:, i] = self.model[0][i].predict_proba(data.iloc[:, 1:].values)[
                    :, 1
                ]
            data["pti"] = np.mean(ret, axis=1)
            data = data[data["pti"] > t1]
            if data.shape[0] > 0:
                ret = np.zeros((data.shape[0], 10))
                for i in range(10):
                    ret[:, i] = self.model[1][i].predict(data.iloc[:, 1:-1].values)
                data.loc[:, "pti"] = np.expm1(np.mean(ret, axis=1))
                data = data[data["pti"] >= t2][["SN", "pti"]].reset_index(drop=True)
                for i in range(data.shape[0]):
                    if data["SN"][i] in fail.keys():
                        data.iloc[i, 1] = min(
                            fail[data["SN"][i]][0] - (ppts - fail[data["SN"][i]][1]),
                            data["pti"][i],
                        )
                    fail[data["SN"][i]] = (data["pti"][i], ppts)
            return data[data["pti"] >= t2]
        else:
